...
{
	public bool CONTROL_A_1;
	public bool CONTROL_B_1;
	public bool CONTROL_X_1;
	public bool CONTROL_Y_1;

	void Start()
	{
		//Debug.Log (CONTROL_A_1);
	}
	...
	void Update()
	{
		...
		if(CONTROL_A_1)	{Jump();}
		if(CONTROL_B_1)	{Back();}
		if(CONTROL_X_1)	{Interact();}
		if(CONTROL_Y_1)	{InstantWin();}
	}

	void ControllerStatus()
	{
		CONTROL_A_1 = GamePad.GetButtonDown(GamePad.Button.A, GamePad.Index.One);
		CONTROL_B_1 = GamePad.GetButtonDown(GamePad.Button.B, GamePad.Index.One);
		CONTROL_X_1 = GamePad.GetButtonDown(GamePad.Button.X, GamePad.Index.One);
		CONTROL_Y_1 = GamePad.GetButtonDown(GamePad.Button.Y, GamePad.Index.One);
	}
    ...
    void Jump()
    {
        ...
        //GamepadState state = GamePad.GetState(GamePad.Index.One); //necessureh to 'get' state
		print ("JUMP!");
        //print("A: " + state.A);
    }

	void Back(){print ("RUN AWAY!");}

	void Interact(){print ("INTERACTION COMMENCING!");}

	void InstantWin(){print ("YOU WON!!");}
